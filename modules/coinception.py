...
class CoInception:
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False):
        ''' Training the CoInception model.
        ...
        not_nan_mask = ~np.isnan(train_data)
        train_data_not_nan = np.nan_to_num(train_data, nan=0)
        train_data_smooth = self.lowpassfilter(train_data_not_nan, thresh=0.1, wavelet="db4")
        train_data_smooth = train_data_smooth[:,:,:not_nan_mask.shape[-1]] * not_nan_mask
        train_data = np.stack([train_data, train_data_smooth], axis=1)
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                out1 = out1[:, -crop_l:]
                out1s = self._net(take_per_row(x_smooth, crop_offset + crop_eleft, crop_right - crop_eleft))
                out1s = out1s[:, -crop_l:]
